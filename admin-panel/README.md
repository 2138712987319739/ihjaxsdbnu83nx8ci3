...
Configure Supabase with the SQL in `supabase/migrations`. Add admin users to `admin_users`, then set the public Supabase URL and anon key in `.env.local`.
...
Main can edit runtime settings for the Bedrock target, session card, presence, friend automation, add-back timing, invite cooldowns, player limits, allowlists, blocklists, and lockdown mode. Developer includes diagnostics, fix logs, action history, security events, lockdown actions, and XUID block controls.
